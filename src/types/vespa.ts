...
export interface VespaHit {
    id: string;
    relevance: number;
    source?: string; // Or specific type if known
    fields?: {
        [key: string]: any;
    };
    children?: VespaHit[];
...
export interface VespaVisitResponse extends VespaResponse {
    ...
    documents?: { id: string; fields: { [key: string]: any } }[];
}
...
export interface VespaDeployResponse extends VespaResponse {
    // Deployment specific fields, e.g., session ID, logs URL
    sessionId?: string;
    logUrl?: string;
    message?: string; // Success or error messages from deployment
}
...
export interface VespaModelEndpointResponse extends VespaResponse {
    // Details about model endpoints
    endpoints?: { modelId: string; url: string }[];
}
...
export interface QueryParams {
    ...
    ranking?: string | { profile?: string; [key: string]: any };
...
export interface DeployOptions {
    ...
    // Add other potential options like disk quota, instance details etc.
    // Based on pyvespa's deploy_to_vespa_cloud parameters
    instance?: string; // e.g., 'default'
    diskLimit?: number;
    memoryLimit?: number;
    vcpuLimit?: number;
}
